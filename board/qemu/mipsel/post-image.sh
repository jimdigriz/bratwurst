#!/bin/sh

set -eu

cd "$1"

[ ! -e vmlinuz -a -f bzImage ] && ln -s bzImage vmlinuz

if [ $(cat vmlinuz | wc -c) -gt $((832*1024)) ]; then
	echo vmlinuz is too large >&2
	exit 1
fi

if [ $(cat rootfs.jffs2 | wc -c) -gt $((3072*1024)) ]; then
	echo rootfs.jffs2 is too large >&2
	exit 1
fi

DD_PARAMS="status=noxfer of=.pflash count=1"

dd $DD_PARAMS conv=sync                      bs=128k  if=/dev/zero	2>/dev/null
dd $DD_PARAMS conv=sync,notrunc oflag=append bs=832k  if=vmlinuz	2>/dev/null
dd $DD_PARAMS conv=sync,notrunc oflag=append bs=3072k if=rootfs.jffs2	2>/dev/null
dd $DD_PARAMS conv=sync,notrunc oflag=append bs=64k   if=/dev/zero	2>/dev/null

mv .pflash pflash

exit 0
